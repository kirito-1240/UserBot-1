...
async def RemoveBG(client , event):
    ...
    if event.reply_to_message and event.reply_to_message.photo:
        ...
        img.seek(0)
        await event.delete()
        await app.send_photo(event.chat.id , "outrem.png" , reply_to_message=event.reply_to_message.message_id , caption="**• Removed Background From Your Photo!**")
        await app.send_sticker(event.chat.id , open("outrem.webp", "rb") , reply_to_message=event.reply_to_message.message_id)
